...
void callback_debug(const BitVector* recorded) 
{
  ...
  {
    RfRemoteController::getPtr()->setNewPacket();
    RfRemoteController::getPtr()->setNewPacket(recorded);
  }
}
...
void RfRemoteController::setNewPacket(const BitVector* packet)
{
    m_recvPacket = getPacket(packet);
}
...
void RfRemoteController::processCode(RemotePacket& packet)
{
    ...
    {
        m_lastHashCount = 1;
        m_lastHashReceived = packet.hash;
    }
    m_newPacket = false;
}
